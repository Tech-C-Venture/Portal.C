      ...
      <div className="w-full max-w-md sm:max-w-lg rounded-xl bg-white px-6 py-8 sm:px-10 sm:py-10 shadow-md text-center">
        ...
        {error && (
          <div className="mt-4 rounded-md bg-red-50 p-3 text-sm text-red-700">
            ログインに失敗しました。<br />
            お使いのアカウントが Tech.C Venture に登録されていない可能性があります。
          </div>
        )}

        <button
          onClick={() => signIn("zitadel", { callbackUrl: "/" })}
          className="mt-8 w-full rounded-lg bg-indigo-600 py-3 text-base sm:text-lg font-semibold text-white hover:bg-indigo-700 transition"
        >
          Tech.C Venture認証システムでログイン
        </button>

        <p className="mt-6 text-xs text-gray-500">
          本システムは Tech.C Venture 関係者専用です
        </p>
      </div>
    </main>
  );
