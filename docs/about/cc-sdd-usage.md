...
### Claude Code用 (12コマンド)
`.claude/commands/kiro/` にコマンドを配置（`spec-quick.md` を含む）
...
**言語ルール:** 生成される Markdown は `spec.json` の `language` に従って記述します。
...
| コマンド | 説明 |
|---------|------|
| `/kiro:spec-quick` | **新規**: 仕様を迅速に作成（クイック実行） |
...
Claude Code Subagentsでは、Claude Codeと同じコマンドに加えてサブエージェントが利用できます。
...
| Quick Spec | `/kiro:spec-quick` | `/kiro:spec-quick` | - | - | - | - |
...
│   ├── settings/
│   │   ├── rules/         # 開発ルール
│   │   └── templates/     # テンプレート（specs/init.jsonを含む）
│   ├── steering/          # プロジェクト知識
│   │   ├── product.md
│   │   ├── tech.md
│   │   └── structure.md
